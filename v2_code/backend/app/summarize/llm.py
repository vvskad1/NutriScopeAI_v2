...
def _groq_structured_summary(context: Dict[str, Any], results: List[Dict[str, Any]]) -> Tuple[bool, Dict[str, Any]]:
    ...
    sys_text = (
        "You are a medical assistant. Given lab results, return a JSON object with these keys: "
        "'per_test' (list of tests, each with: test, value, unit, status, importance, why_low, why_high, risks_if_low, risks_if_high, next_steps), "
        "'diet_plan' (object with add/limit lists), and 'overall_message' (optional). "
        "For each test flagged as low or high, provide: why it is important, why it may be low/high, risks if neglected, and how to improve. "
        "Separate clearly in the JSON: which tests are low, which are high, and which are normal (use the 'status' field). "
        "Do NOT return any prose summary or paragraph, only the structured JSON. "
        "Use the provided KB info (importance, common reasons, advice). No diagnoses."
    )
